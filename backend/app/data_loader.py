...
from app.prompts import build_insight_and_charting_prompt, build_synthesis_prompt
...
def call_llm_with_retry(prompt, max_retries=3):
    """Call LLM with exponential backoff retry for 503 errors"""
        ...
        try:
            ...
            if '503' in error_msg or 'overloaded' in error_msg or 'unavailable' in error_msg:
                if attempt < max_retries - 1:
                    ...
                    logging.warning(f"API overloaded (attempt {attempt + 1}), retrying in {wait_time:.1f}s...")
...
def optimized_single_analysis(user_question: str, df: pd.DataFrame):
    ...
    # For datasets over 600 rows, take a smart sample to fit context window
    if len(df) > 600:
        # Take recent data first, then sample if still too large
        if 'month' in df.columns:
            df_sorted = df.sort_values('month', ascending=False)
            sample_df = df_sorted.head(500)
        else:
    ...
    try:
        ...
        if not llm_response_str:
            return {"text_answer": "Unable to process request due to service issues. Please try again.", "charts": []}
        ...
        if 'charts' in result:
            logging.info(f"Chart specs found: {len(result['charts'])}")
            logging.info(f"Chart spec types: {[type(spec) for spec in result['charts']]}")
        ...
        logging.error(f"JSON parsing error in optimized analysis: {e}")
        logging.error(f"Problematic response: {llm_response_str[:1000] if llm_response_str else 'None'}")
        return {"text_answer": "Analysis completed but response formatting failed. Please try again.", "charts": []}
    except Exception as e:
        logging.error(f"Optimized analysis failed: {e}")
        return {"text_answer": f"Analysis error: {str(e)}", "charts": []}
